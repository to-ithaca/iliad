...
object Load {
  ...
  def newBuffer(v: Model.VertexData, pageSize: Int): DSL[VertexBuffer.Loaded] =
    LoadNewVertexBuffer(v, pageSize).free
  def newBuffer(
      e: Model.ElementData, pageSize: Int): DSL[ElementBuffer.Loaded] =
    LoadNewElementBuffer(e, pageSize).free
...
case class LoadNewVertexBuffer(v: Model.VertexData, pageSize: Int)
    extends Load[VertexBuffer.Loaded]
case class LoadNewElementBuffer(e: Model.ElementData, pageSize: Int)
  ...
  def apply[A](load: Load[A]): GL.DSL[A] = load match {
    ...
    case LoadCopyVertexBuffer(v, pageSize, b) =>
      ...
      GL.copyVertices(b.id, b.filled, v.size, v.data, capacity)
        .map(VertexBuffer.Loaded(_, b.filled + v.size, capacity))
    case LoadCopyElementBuffer(e, pageSize, b) =>
      val capacity = roundUp(e.size, pageSize)
      GL.copyElements(b.id, b.filled, e.size, e.data, capacity)
        .map(ElementBuffer.Loaded(_, b.filled + e.size, capacity))
  }
}
