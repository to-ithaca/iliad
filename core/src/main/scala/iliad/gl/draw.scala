...
object Draw {
  ...
  def bindFramebuffer(framebuffer: Int): DSL[Unit] = BindFramebuffer(framebuffer).free
  ...
  def enable(as: Attribute.LoadedAttributes, baseOffset: Int): DSL[Unit] = EnableAttributes(as, baseOffset).free
...
case class EnableAttributes(as: Attribute.LoadedAttributes, baseOffset: Int) extends Draw[Unit]
...
private object DrawParser extends (Draw ~> GL.DSL) {
  ...
  private def enableAttribute(stride: Int)(a: Attribute.Offset): GL.DSL[Unit] = 
    GL.enableAttribute(a.l.location, a.l.c.elementSize, a.l.c.`type`, stride, a.offset)
  ...
  def apply[A](draw: Draw[A]): GL.DSL[A] = draw match {
    ...
    case DrawTriangleModel(range: DataRange) => GL.drawTriangles(range.start, range.end)
