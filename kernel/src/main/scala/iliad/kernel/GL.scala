...
abstract class GL[F[_]: Monad] {
  ...
  private def createFramebuffer(framebuffer: Framebuffer, outputs: List[LoadedShaderOutput]): IO[F, Int] = genFramebuffer >>= { id => 
  ...
  private def existingFramebuffer(f: Framebuffer): IO[F, Option[LoadedFramebuffer]] = inspect(s => _framebuffers.get(s).find(ff => ff.framebuffer == f))

  private def loadedOutput(page: LoadedTexture => LoadedTexturePage)(o: ShaderOutputInstance): IO[F, LoadedShaderOutput] = o match {
    case t: TextureInstance => getExistingTexture(t).map(page)
    case r: RenderbufferInstance => renderbuffer(r).map(identity)
  }

  private def loadedOutputs(page: LoadedTexture => LoadedTexturePage)(os: List[ShaderOutputInstance]): IO[F, List[LoadedShaderOutput]] = keyTraverse(os)(loadedOutput(page)) map (_.map(_._2))
  
  private def createFramebuffer(page: LoadedTexture => LoadedTexturePage)(f: Framebuffer): IO[F, Int] = loadedOutputs(page)(f.outputs.map(_._2)) >>= { os =>
    createFramebuffer(f, os)
  }

  private def createFramebuffer(f: Framebuffer): IO[F, LoadedFramebuffer] = modify(if(f.isDouble) for {
  ...
  def newBuffer(target: BufferTarget, capacity: Int): IO[F, Int] = genBuffer >>= { id =>
    bindBuffer(target, id) >> bufferData(target, capacity, null, GL_STATIC_DRAW) map (_ => id)
  }

  def initialBuffer(buffer: BufferInstance, target: BufferTarget, data: Buffer, size: Int, capacity: Int): IO[F, LoadedBuffer] = modify(newBuffer(target, capacity) >>= { id =>
    bufferSubData(target, 0, size, data) map (_ => LoadedBuffer(buffer, id, capacity, size, target))
  })(b => _buffers.modify(b :: _))

  def insertInBuffer(buffer: LoadedBuffer, data: Buffer, size: Int): IO[F, LoadedBuffer] = modify(bindBuffer(buffer.target, buffer.id) >> 
    bufferSubData(buffer.target, buffer.filled, size, data) map (_ => buffer.add(size)))(b => _buffers.modify(_.map { 
      case bb if (bb == buffer) => b
      case x => x
}))

  def copyToNew(buffer: LoadedBuffer, data: Buffer, size: Int, capacity: Int): IO[F, LoadedBuffer] = modify(newBuffer(buffer.target, capacity) >>= { id =>
  ...
  def existingBuffer(bufferInstance: BufferInstance, target: BufferTarget): IO[F, Option[LoadedBuffer]] = inspect(s =>_buffers.get(s).find(b => b.instance == bufferInstance && b.target == target))
...
object MissingCatsImplicits {
  ...
  def stateTFunctor[F[_], S](implicit F: Functor[F]): Functor[StateT[F, S, ?]] = new Functor[StateT[F, S, ?]] {
    def map[A, B](fa: StateT[F, S, A])(f: A => B): StateT[F, S, B] = new StateT(fa.runF.map(fsa => s => fsa(s).map {
      case (s, a) => (s, f(a))
    }))
  }
}
...
object GL {
  ...
  object UniformType {
    case object Mat4f extends UniformType
  }


  sealed trait SamplerType
  object SamplerType {
    case object IntSampler extends SamplerType
    case object FloatSampler extends SamplerType
  }
  ...
  sealed trait BufferedId
  object BufferedId {
    case class Single(id: Int) extends BufferedId
  ...
  case class LoadedTexture(instance: TextureInstance, id: BufferedId) {
    def frontPage: LoadedTexturePage = ???
    def backPage: LoadedTexturePage = ???
  }
  ...
  object ChannelBitMask {
    ...
    case class BitMask(each: Set[ChannelBit]) extends ChannelBitMask {
      override def toString = s"(${each.mkString("|")})"
    }
  }
