...
abstract class GL[F[_]] {
  ...
  //TODO def colorMask(red: Boolean, green: Boolean, blue: Boolean, alpha: Boolean): IO[F, Unit]
  ...
  private def getShaderiv[A](shader: Int, pname: ShaderParameter, expected: Set[A])(eq: A => Int)(implicit FF: Functor[F]): IO[F, A] = getShaderiv(shader, pname).map(code => expected.find(eq(_) == code).get)

  def getShaderiv(shader: Int, pname: GL_SHADER_TYPE.type)(implicit FF: Functor[F]): IO[F, ShaderType] = getShaderiv(shader, pname, SealedEnum.values[ShaderType])(_.value)
  def getShaderiv(shader: Int, pname: GL_DELETE_STATUS.type)(implicit FF: Functor[F]): IO[F, Boolean] = getShaderiv(shader, pname, SealedEnum.values[TrueFalse])(_.value).map(_ == GL_TRUE)
  def getShaderiv(shader: Int, pname: GL_COMPILE_STATUS.type)(implicit FF: Functor[F]): IO[F, Boolean] = getShaderiv(shader, pname, SealedEnum.values[TrueFalse])(_.value).map(_ == GL_TRUE)
  def getShaderiv(shader: Int, pname: GL_INFO_LOG_LENGTH.type)(implicit FF: Functor[F]): IO[F, Int] = getShaderiv(shader, pname: ShaderParameter)
  def getShaderiv(shader: Int, pname: GL_SHADER_SOURCE_LENGTH.type)(implicit FF: Functor[F]): IO[F, Int] = getShaderiv(shader, pname: ShaderParameter)
