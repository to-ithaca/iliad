...
private[kernel] final class GLDebugger[F[_]](
    gl: GL[F])(implicit M: Monad[F], val MM: Monad[DebugEffect[F, ?]])
    extends GL[DebugEffect[F, ?]] {
  ...
  private def lift[A](io: IO[F, A]): IO[DebugEffect[F, ?], A] =
    io.mapF(_.transformF(_.liftT[DebugEffect]))

  private def error[A](method: String): IO[DebugEffect[F, ?], A => A] =
    getError.mapF(
        _.transformF(_.transform {
  ...
  private def debug[A](io: IO[F, A])(
      method: String): IO[DebugEffect[F, ?], A] = lift(io).ap(error(method))
  ...
  def enable(cap: Capability): IO[DebugEffect[F, ?], Unit] =
    debug(gl.enable(cap))(s"glEnable($cap)")
  def colorMask(red: Boolean,
                green: Boolean,
                blue: Boolean,
                alpha: Boolean): IO[DebugEffect[F, ?], Unit] =
    debug(gl.colorMask(red, green, blue, alpha))("glColorMask")

  def activeTexture(texture: Int): IO[DebugEffect[F, ?], Unit] =
  ...
  def bindFramebuffer(target: FramebufferTarget,
                      framebuffer: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.bindFramebuffer(target, framebuffer))("glBindFramebuffer")
  def bindRenderbuffer(renderbuffer: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.bindRenderbuffer(renderbuffer))("glBindRenderbuffer")
  def bindTexture(
  ...
  def bufferData(target: BufferTarget,
                 ...
                 usage: BufferUsage): IO[DebugEffect[F, ?], Unit] =
    debug(gl.bufferData(target, size, data, usage))("glBufferData")
  def bufferSubData(target: BufferTarget,
                    offset: Int,
                    size: Int,
                    data: java.nio.Buffer): IO[DebugEffect[F, ?], Unit] =
    debug(gl.bufferSubData(target, offset, size, data))("glSubBufferData")
  def checkFramebufferStatus(
      target: FramebufferTarget): IO[DebugEffect[F, ?], FramebufferStatus] =
  ...
  private[kernel] def clearBufferfv(
      target: Channel,
      drawBuffer: Int,
      value: Array[Float]): IO[DebugEffect[F, ?], Unit] =
    debug(gl.clearBufferfv(target, drawBuffer, value))("glClearBufferfv")
  private[kernel] def clearBufferiv(
      target: Channel,
      drawBuffer: Int,
      value: Array[Int]): IO[DebugEffect[F, ?], Unit] =
    debug(gl.clearBufferiv(target, drawBuffer, value))("glClearBufferiv")
  private[kernel] def clearBufferuiv(
      target: Channel,
      drawBuffer: Int,
      value: Array[Int]): IO[DebugEffect[F, ?], Unit] =
    debug(gl.clearBufferuiv(target, drawBuffer, value))("glClearBufferuiv")
  def clearColor(red: Float,
                 green: Float,
                 blue: Float,
                 alpha: Float): IO[DebugEffect[F, ?], Unit] =
    debug(gl.clearColor(red, green, blue, alpha))("glClearColor")
  def compileShader(shader: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.compileShader(shader))("glCompileShader")
  def copyBufferSubData(read: BufferTarget,
                        write: BufferTarget,
                        readOffset: Int,
                        writeOffset: Int,
                        size: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.copyBufferSubData(read, write, readOffset, writeOffset, size))(
        "glCopyBufferSubData")
  def createProgram: IO[DebugEffect[F, ?], Int] =
  ...
  def drawElementsInstanced(mode: PrimitiveType,
    ...
    debug(gl.drawElementsInstanced(mode, count, `type`, offset, primCount))(
        "glDrawElementsInstanced")
  def enableVertexAttribArray(location: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.enableVertexAttribArray(location))("glEnableVertexAttribArray")
  def framebufferRenderbuffer(target: FramebufferTarget,
                              attachment: FramebufferAttachment,
                              renderbuffer: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.framebufferRenderbuffer(target, attachment, renderbuffer))(
        "glFramebufferRenderbuffer")
  def framebufferTexture2D(target: FramebufferTarget,
                           attachment: FramebufferAttachment,
                           texTarget: FramebufferTexTarget,
                           texture: Int,
                           level: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.framebufferTexture2D(
            target, attachment, texTarget, texture, level))(
        "glFramebufferTexture2D")
  private[kernel] def genBuffers(num: Int): IO[DebugEffect[F, ?], Array[Int]] =
    debug(gl.genBuffers(num))("glGenBuffers")
  private[kernel] def genFramebuffers(
      num: Int): IO[DebugEffect[F, ?], Array[Int]] =
    debug(gl.genFramebuffers(num))("glGenFramebuffers")
  private[kernel] def genRenderbuffers(
      num: Int): IO[DebugEffect[F, ?], Array[Int]] =
    debug(gl.genRenderbuffers(num))("glGenRenderbuffers")
  private[kernel] def genSamplers(
      num: Int): IO[DebugEffect[F, ?], Array[Int]] =
    debug(gl.genSamplers(num))("glGenSamplers")
  private[kernel] def genTextures(
      num: Int): IO[DebugEffect[F, ?], Array[Int]] =
    debug(gl.genTextures(num))("glGenTextures")
  def getAttribLocation(
      program: Int, name: String): IO[DebugEffect[F, ?], Int] =
    debug(gl.getAttribLocation(program, name))("glGetAttribLocation")
  private[kernel] def getShaderInfoLog(
      shader: Int, maxLength: Int): IO[DebugEffect[F, ?], String] =
    debug(gl.getShaderInfoLog(shader, maxLength))("glGetShaderInfoLog")
  private[kernel] def getShaderiv(
      shader: Int, pname: ShaderParameter): IO[DebugEffect[F, ?], Int] =
    debug(gl.getShaderiv(shader, pname))("glGetShaderiv")
  ...
  def renderbufferStorage(format: RenderbufferInternalFormat,
    ...
    debug(gl.renderbufferStorage(format, width, height))(
        "glRenderbufferStorage")
  private[kernel] def samplerParameteri(
      sampler: Int,
      name: SamplerParameter,
      value: Int): IO[DebugEffect[F, ?], Unit] =
    debug(gl.samplerParameteri(sampler, name, value))("glSamplerParameteri")
  def shaderSource(shader: Int,
                   count: Int,
                   sources: Seq[String]): IO[DebugEffect[F, ?], Unit] =
    debug(gl.shaderSource(shader, count, sources))("glShaderSource")
