...
private[gl] final class Logging[F[_]](config: LoggerConfig, gl: GL[F])(implicit F: Functor[F], val S: Semigroup[Logger[F, Unit]], val M: Monad[Logger[F, ?]]) extends GL[Logger[F, ?]] {
  ...
  private[gl] def samplerParameteri(sampler: Int, name: SamplerParameter, value: IntConstant): IO[Logger[F, ?], Unit] = log(gl.samplerParameteri(sampler, name, value))(s"glSamplerParameteri($sampler, $name, $value)")
  ...
  private[gl] def texParameteri(target: TextureTarget, name: TextureParameter, value: IntConstant): IO[Logger[F, ?], Unit] = log(gl.texParameteri(target, name, value))(s"glTexParameteri($target, $name, $value)")
