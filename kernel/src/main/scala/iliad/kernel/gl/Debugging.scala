...
private[gl] final class Debugging[F[_]](gl: GL[F])(implicit A: Applicative[F], val S: Semigroup[Debugger[F, Unit]], val M: Monad[Debugger[F, ?]]) extends GL[Debugger[F, ?]] {
  ...
  private def error[A](method: String): IO[Debugger[F, ?], A => A] = getError.mapF(_.transform {
  ...
  private[gl] def samplerParameteri(sampler: Int, name: SamplerParameter, value: IntConstant): IO[Debugger[F, ?], Unit] = debug(gl.samplerParameteri(sampler, name, value))("glSamplerParameteri")
  ...
  private[gl] def texParameteri(target: TextureTarget, name: TextureParameter, value: IntConstant): IO[Debugger[F, ?], Unit] = debug(gl.texParameteri(target, name, value))("glTexParameteri")
