...
class EGLRunner[NDisp, NWin, Disp, Cfg: ClassTag, Sfc, Ctx]
    extends EGL[Id, NDisp, NWin, Disp, Cfg, Sfc, Ctx] {
  import EGL._
  ...
  def getError: IO[Option[Int Xor ErrorCode]] =
    Reader(
      ...
      case value =>
        Some(Xor.fromOption(
                SealedEnum.values[ErrorCode].find(_.value == value), value))
    })
  def getConfigAttrib(
  ...
  def queryString(dpy: Disp, name: QueryKey): IO[String] =
    Reader(egl => egl.eglQueryString(dpy, name.value))

  def getDisplay(displayID: NDisp): IO[Disp] =
    Reader(_.eglGetDisplay(displayID))
  ...
  def createWindowSurface(display: Disp,
    ...
    Reader(_.eglCreateWindowSurface(display, config, win, attributes.toArray))
  def createPbufferSurface(
      display: Disp, config: Cfg, attributes: PBufferAttributes): IO[Sfc] =
    Reader(_.eglCreatePbufferSurface(display, config, attributes.toArray))
  def bindApi(api: API): IO[Unit] = Reader(_.eglBindAPI(api.value))

  def createContext(display: Disp,
                    config: Cfg,
                    shareContext: Ctx,
                    attributes: ContextAttributes): IO[Ctx] =
    Reader(
        _.eglCreateContext(display, config, shareContext, attributes.toArray))

  def swapBuffers(display: Disp, surface: Sfc): IO[Unit] =
    Reader(_.eglSwapBuffers(display, surface))
  def makeCurrent(
      display: Disp, draw: Sfc, read: Sfc, context: Ctx): IO[Unit] =
    Reader(_.eglMakeCurrent(display, draw, read, context))
}
