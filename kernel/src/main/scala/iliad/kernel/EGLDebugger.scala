...
class EGLDebugger[F[_], NDisp, NWin, Disp, Cfg: ClassTag, Sfc, Ctx](val egl: EGL[F, NDisp, NWin, Disp, Cfg, Sfc, Ctx])(implicit val M: Monad[F]) extends EGL[EGL.DebugEffect[F, ?], NDisp, NWin, Disp, Cfg, Sfc, Ctx] {
  ...
  def getConfigAttrib(display: Disp, config: Cfg, attribute: FixedConfigAttrib): IO[Int] = debug("getConfigAttrib")(egl.getConfigAttrib(display, config, attribute))
  def queryString(dpy: Disp, name: QueryKey): IO[String] = debug("queryString")(egl.queryString(dpy, name))
  ...
  private[kernel] def createContext(d: Disp, cfg: Cfg, ctx: Ctx, as: ContextAttributes): IO[Ctx] = debug("createContext")(egl.createContext(d, cfg, ctx, as))

  //TODO: enum error codes
  def swapBuffers(display: Disp, surface: Sfc): IO[Unit] = debug("swapBuffers")(egl.swapBuffers(display, surface))
  def makeCurrent(display: Disp, draw: Sfc, read: Sfc, context: Ctx): IO[Unit] = debug("makeCurrent")(egl.makeCurrent(display, draw, read, context))
}
